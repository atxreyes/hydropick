...
class SurveyLine(HasTraits):
    ...
    #==========================================================================
    # REMAINING TRAITS ARE USER GENERATED  - NEED TO BE SAVED WHEN CHANGED
    #==========================================================================

    #: depth of the lake at each location as generated by various sources
    ...
    # reference depth line name for the relative core boundaries plotting
    core_depth_reference_str = Str(CURRENT_SURFACE_FROM_BIN_NAME)

    # project directory where this survey line will save itself
    project_dir = Str

    #==========================================================================
    # PROPERTY TRAITS - NO NEED TO SAVE
    #==========================================================================

    # indicated if mask has been defined for this line
    masked = Property(Bool, depends_on='mask')

    # reference depth line for the relative core boundaries plotting
    core_depth_reference = Property(List, depends_on=['core_depth_ref_string'])

    def _get_core_depth_reference(self):
        name = self.core_depth_reference_str
        if name == 'Final Lake Depth':
            name = self.final_lake_depth

        if name in self.lake_depths:
            dline_name = name
        elif self.final_lake_depth in self.lake_depths:
            name = self.final_lake_depth
        elif CURRENT_SURFACE_FROM_BIN_NAME in self.lake_depths:
            dline_name = CURRENT_SURFACE_FROM_BIN_NAME
        elif len(self.lake_depths) > 0:
            dline_name = self.lake_depths.keys()[0]
        else:
            dline_name = None

        if dline_name is not None:
            dline = self.lake_depths[dline_name]
        else:
            dline = None
    ...
    def _set_core_depth_reference(self, name):
        print 'setting core ref', name
        choices = ['Final Lake Depth'] + self.lake_depths.keys()
        if name in choices:
            self.core_depth_reference_str = name
        elif CURRENT_SURFACE_FROM_BIN_NAME in choices:
            self.core_depth_reference_str = CURRENT_SURFACE_FROM_BIN_NAME
        else:
            self.core_depth_reference_str = None
    ...
    def save_to_disk(self, project_dir=None):
        ...
        '''
        # note: deferred to avoid circular import
        ...
        if os.path.isdir(project_dir):
            logger.warn('this would save survey line {} to disk if it could'
                        .format(self.name))
            survey_io.write_survey_line_to_hdf(project_dir, self)
        else:
